...
class RouteProvider
{
	...
	public function autoRegister(RouteBuilder $builder, bool $clearCache = false): void
	{
		...
		$routes
			...
			->each(function (array $scopedRoutes, string $scope) use ($builder) {
				$scope = Router::normalize("/$scope/");
				$builder->scope($scope, function (RouteBuilder $routes) use ($scopedRoutes) {
